// src/historial_adopciones/historial_adopciones.controller.ts
...
export class HistorialAdopcionesController { // El nombre del controlador puede mantenerse en plural
  ...
  @Get(':id')
  findOne(@Param('id', ParseUUIDPipe) id: string): Promise<HistorialAdopcione> {
    return this.historialAdopcionesService.findOne(id);
  }

  @Patch(':id')
  update(@Param('id', ParseUUIDPipe) id: string, @Body() updateHistorialAdopcioneDto: UpdateHistorialAdopcioneDto): Promise<HistorialAdopcione> {
    return this.historialAdopcionesService.update(id, updateHistorialAdopcioneDto);
  }

  @Delete(':id')
  @HttpCode(HttpStatus.NO_CONTENT)
  remove(@Param('id', ParseUUIDPipe) id: string): Promise<HistorialAdopcione> {
